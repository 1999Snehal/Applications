...
void man(char *name)
{
    ...
    else if(strcmp(nmae, "closeall") == 0)
    {
...
int CreateFile(char *name, int permission)
{
   ...
   if(Get_Inode(name) != NULL)
    ...
    UFDTArr[i].ptrfiletable->made = permission;
...
if(read_size < isize)
{
    strncpy(arr,(UFDTArr[fd].ptrfiletable->ptrinode->Buffer) + (UFDTArr[fd].ptrfiletable->readoffset),read < read_size)

    UFDTArr[fd].ptrfiletable->readoffset=UFDTArr[fd].ptrfiletable->readoffset + read_size;
}
else
{
     strncpy(arr,(UFDTArr[fd].ptrfiletable->ptrinode->Buffer) + (UFDTArr[fd].ptrfiletable->readoffset),read < isize)
...
int LseekFile(inf fd, int size, int from)
{
    ...
    else if(UFDTArr[fd].ptrfiletable->mode == WRITE)
    {
        ...
        else if(from == END)
        {
            if((UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) + size > MAXFILESIZE)   return -1;
            if(((UFDTArr[fd].ptrfiletable->writeoffset) + size) < 0)  return -1;
            (UFDTArr[fd].ptrfiletable->writeoffset) = (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize) _ size;
        }
    }
}
...
void ls_file()
{
    ...
    printf("\nFile Name\tInodee number\tFile size\tLink count\n");
    printf("-----------------------------------------------------------\n");
    while(temp != NULL)
    {
        if(temp->FileType != 0)
